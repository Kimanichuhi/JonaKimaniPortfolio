...
export default function ValuesEditor() {
  const { items, loading, saving, save, remove } = useCrud<Value>('values');
  const [editing, setEditing] = useState<Partial<Value>>({});

  if (loading) return <LoadingState />;

  const startAdd = () => setEditing({ title: '', description: '', icon: 'Lightbulb', sort_order: items.length + 1 });
  const startEdit = (item: Value) => setEditing({ ...item });

  return (
    <div>
      <SectionHeader title="Core Values" onAdd={startAdd} />
      {Object.keys(editing).length > 0 && (
        <ItemCard onDelete={editing.id ? () => { remove(editing.id!); setEditing({}); } : undefined} saving={saving}>
          <div className="grid sm:grid-cols-3 gap-4">
            ...
            <InputField label="Icon" value={editing.icon ?? 'Lightbulb'} onChange={v => setEditing(p => ({ ...p, icon: v }))} placeholder="Lightbulb, Heart, Globe, Users, BookOpen, Award" />
      ...
      {items.map(item => (
        <div key={item.id} className="flex items-center justify-between bg-gray-900 border border-gray-800 rounded-lg p-4 mb-2">
          <div><span className="font-medium text-white">{item.title}</span> <span className="text-gray-500 text-sm">({item.icon})</span><p className="text-gray-400 text-sm mt-1">{item.description}</p></div>
