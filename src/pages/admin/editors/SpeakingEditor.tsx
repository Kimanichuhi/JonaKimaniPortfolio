...
import type { SpeakingEvent } from '../../../lib/dataCache';

export default function SpeakingEditor() {
  const { items, loading, saving, save, remove } = useCrud<SpeakingEvent>('speaking_events');
  const [editing, setEditing] = useState<Partial<SpeakingEvent>>({});

  if (loading) return <LoadingState />;

  const startAdd = () => setEditing({ title: '', date: '', location: '', type: 'Keynote', description: '', video_url: '', sort_order: items.length + 1 });
  const startEdit = (item: SpeakingEvent) => setEditing({ ...item });

  return (
    <div>
      <SectionHeader title="Speaking Events" onAdd={startAdd} />
      {Object.keys(editing).length > 0 && (
        <ItemCard onDelete={editing.id ? () => { remove(editing.id!); setEditing({}); } : undefined} saving={saving}>
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <InputField label="Date" value={editing.date ?? ''} onChange={v => setEditing(p => ({ ...p, date: v }))} placeholder="March 15, 2026" />
            <InputField label="Location" value={editing.location ?? ''} onChange={v => setEditing(p => ({ ...p, location: v }))} />
            <InputField label="Type" value={editing.type ?? 'Keynote'} onChange={v => setEditing(p => ({ ...p, type: v }))} placeholder="Keynote, Panel, Talk, Workshop, Fireside Chat" />
            <InputField label="Video URL" value={editing.video_url ?? ''} onChange={v => setEditing(p => ({ ...p, video_url: v }))} />
      ...
      {items.map(item => (
        <div key={item.id} className="flex items-center gap-4 bg-gray-900 border border-gray-800 rounded-lg p-4 mb-2">
          <div className="flex-1 min-w-0">
            <div className="font-medium text-white">{item.title} <span className="text-teal-400 text-xs">({item.type})</span></div>
            <div className="text-gray-400 text-sm">{item.date} • {item.location}</div>
          </div>
